...
inline std::string ActionName(Action action) {
    switch (action) {
    case Action::AttackLowestEnemy:
        return "AttackLowestEnemy";
    case Action::MoveLowestAllyBack:
        return "MoveLowestAllyBack";
...
public:
    ...
    CombatState state;
    std::vector<CombatUnit*> units_1;
    std::vector<CombatUnit*> units_2;
    int player;
    int max_x = 15;
    int max_y = 15;
    int min_x = 0;
    ...
    // constructor
    Game();
    Game(const std::vector<CombatUnit>& units, int player);

    // methods
    void addUnit(CombatUnit unit);
    bool executeAction(Action,bool verbose=false);
    void update();
    int isWin();
    int rollout(bool time_limit=false);
};
...
class Zobrist {
public:
    std::map<std::pair<uint64_t, int>, int> wins; // stores number of wins when in a particular configuration of the game
    std::map<std::pair<uint64_t, int>, int> playouts; // stores number of playouts done in a particular configuration of the game
};
...
class Mcts {
public:
    std::vector<std::pair<Action, std::pair<int, int>>> moves_stats;
    Zobrist zobrist;

    Mcts();
    Action flat_mcts(Game game, int playouts,bool verbose=false, bool time_limit=false);
    Action uct_mcts(Game game, int playouts, float c, bool verbose=false);
};
...
class DefeatRoachesMctsBot : public Agent{
public:
    virtual void OnGameStart() final;
    virtual void OnStep() final;
    
private:
    bool GetUnitPosition(UNIT_TYPEID unit_type, int unit_id, Unit::Alliance alliance, Point2D& position);
    bool GetGroupPosition(UNIT_TYPEID unit_type, Unit::Alliance alliance, Point2D& position);
    bool GetLowestAlly();
    bool GetLowestEnemy();
